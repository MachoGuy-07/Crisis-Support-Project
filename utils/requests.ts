...
export function canVolunteerAcceptRequest(
  ...
  if (request.supplyType === "food") return supply.foodAmount >= request.itemsCount;
  if (request.supplyType === "medical") return supply.medicalAmount >= request.itemsCount;
  if (request.supplyType === "shelter") return supply.shelterAmount >= request.itemsCount;

  return supply.customDescription.trim().length > 5;
}
